...
vec3 getBackground(vec3 dir) {
    ...
    uv.y = phi / 3.14159265;
    return texture(u_skybox, uv).rgb*0.3;
}
...
void main() {
    ...
    for(int i = 0; i < MAX_STEPS; i++) {
        ...
        if(distToPlane < 0.2 && r > DISK_INNER && r < DISK_OUTER) {
            ...
            float verticalFade = 1.0 - (distToPlane / 0.2);
            float density = gas * radialFade * verticalFade * 0.2;

            vec3 diskVel = normalize(vec3(-rayPos.z, 0.0, rayPos.x));
            float doppler = dot(diskVel, rayDir);
        ...
        rayPos += rayDir * STEP_SIZE * min(r, 5.0);

        if(r > 30.0) break;
    }

    if(!hitEventHorizon) {
        finalColor = getBackground(rayDir);
    }
...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(1200, 900, "Ton618 Black Hole Simulation", NULL, NULL);
    ...
    unsigned int milkyWayTex = loadTexture("MilkyWay.jpg");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniform1i(glGetUniformLocation(shaderProgram, "u_skybox"), 0);
